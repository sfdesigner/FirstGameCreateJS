...
function buildGameGrid() {
    ...
    statusText = new createjs.Text("Defeat the dragon!", "12px Arial", "#FFF");
    statusText.x = 627;
    statusText.y = 350;
...
function drawControls() {
    ...
    attackButton = new createjs.Shape();
    attackButton.graphics.beginFill("red").drawCircle(702, 260, 50);
    attackButton.shadow = new createjs.Shadow("black", 0, 0, 10);
    attackButton.onClick = avatarAttack;
...
function avatarAttack() {
    var inRange = false;
    if (Math.abs(playerLocation.x - 10) <= 1 && Math.abs(playerLocation.y - 10) <= 1) inRange = true;
    
    if (inRange) {
        var stringBuilder = "You attack...";
        if (Math.random() < .8) {
            playerHealth -= 5;
            stringBuilder += "\n...and are hit by the dragon!";
        }
        if (Math.random() < .7) {
            enemyHealth -= 5;
            stringBuilder += "\n...and you hit the dragon!";
        }

        updateHealthBars();
        updateStatusText(stringBuilder);

        if (playerHealth <= 0) {
            attackButton.graphics.clear();
            gameGrid[playerLocation.y][playerLocation.x].gotoAndStop(gridData[playerLocation.y][playerLocation.x]);
            updateStatusText("You are dead :(");
        }

        if (enemyHealth <= 0) {
            attackButton.graphics.clear();
            gameGrid[10][10].gotoAndStop(gridData[10][10]);
            updateStatusText("You are victorious!");
        }
    } else {
        updateStatusText("You are too far away!");
    }
}
